...
- **FR-002**: The repository MUST contain a public header file with the full `AzureTableClient` class declaration — all method signatures from the public API contract (constructors, `SetBearerToken`, `CreateTableIfNotExists`, entity CRUD, batch, query, delete, and all async variants) plus the necessary includes (`libazure-storage-client/json.hpp`, `<string>`, `<vector>`, `<functional>`). Method bodies are stubs (unimplemented). This header is the file that gets installed for downstream consumers.
